...
// TODO: Descomentar cuando api.js esté listo
// import {
//     getEventosAbiertos,
//     getEventosPorTipo,
//     getEventosPorDia,
...
class SantoroIntegrator {
  ...
  async buscarErrores(parametros) {
    ...
    return {
      exito: true,
      mensaje:
        'Para buscar errores necesito más información. ¿Podrías darme un código de error específico?',
      datos: null,
  ...
  async obtenerEventos(parametros) {
    ...
    // const { filtros } = parametros // TODO: usar cuando api.js esté listo
    ...
    try {
      ...
      // Temporal: devolver datos mockeados
      resultado = []
      mensaje = `Eventos para periodo "${periodo}" (función temporal)`
  ...
  analizarUsuarios(datos) {
    const usuarios = datos
      .map((item) => item.person?.nombreCompleto || item.usuario)
      .filter(Boolean)
