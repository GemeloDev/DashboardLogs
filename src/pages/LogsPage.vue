            ...
            <q-card style="background-color: #1e1e2f;">
              <q-card-section>
                ...
                <div class="chart-wrapper">
                  <canvas ref="funcionalidadesChartRef" style="height: 420px; max-width: 100%;"></canvas>
                </div>
              </q-card-section>
            </q-card>
...
import { getUsuariosOffline, getUsuariosDispositivosDia, getValidadosTFLite, getPorcentajeOffline, getTiempoRespuestaPromedio, getOvalAlineado, getFuncionalidadesMasUsadas } from 'src/services/api'

import LogFilters from 'components/LogFilters.vue'
import LogTable from 'components/LogTable.vue'

const logs = ref([])
const filtroFechas = ref({ start_date: '', end_date: '' })
...
async function onFilter({ start_date, end_date }) {
  await cargarValidadosTFLITE(start_date, end_date)
  ...
  filtroFechas.value = { start_date, end_date }
  ...
  try {
    ...
    const respuestaDispositivos = await getUsuariosDispositivosDia({ fecha: start_date })
    console.log('Respuesta dispositivos:', respuestaDispositivos)
    logs.value = respuestaDispositivos
...
async function cargarValidadosTFLITE(start_date, end_date) {
  try {
    ...
    validadosTFLITE.value = Array.isArray(respuestaValidados) && respuestaValidados.length > 0
...
async function cargarPorcentajeOffline(start_date, end_date) {
  try {
    ...
    porcentajeOffline.value = Array.isArray(respuesta) && respuesta.length > 0
      ? Number(respuesta[0].porcentaje)
      : 0
