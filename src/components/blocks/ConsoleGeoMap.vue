...
function addLayers() {
  ...
  map.addLayer({
    ...
    type: 'circle',
    source: 'geo',
    layout: { visibility: mode.value === 'points' ? 'visible' : 'none' },
    paint: {
  ...
  map.addLayer({
    ...
    type: 'heatmap',
    source: 'geo',
    layout: { visibility: mode.value === 'heat' ? 'visible' : 'none' },
    paint: {
...
function updateMode() {
  ...
  // Cambiar el estilo base del mapa y re-agregar las capas de datos
  loaded = false
